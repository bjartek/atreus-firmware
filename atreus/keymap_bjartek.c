...
const uint16_t PROGMEM actionmaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  KEYMAP( /* hold space brings up move pad and numpad */
    ...
    ACK(DEL), GSK(Q),OSM(LCTL), OSM(LSFT), GUI(D) ,OSM(LGUI), OSM(LALT),  TRNS,    OFF(1), DOT, 0, KP_EQUAL
  ),
  KEYMAP( /* hold tab to have fpad and mouse */
    ...
    AS,     CA,    ACS, OSM(LSFT),    CS, OSM(LGUI), OSM(LALT),    SPC  , TRNS   , MINS , SH(NO_BSLS), KP_EQUAL
  ),
};
